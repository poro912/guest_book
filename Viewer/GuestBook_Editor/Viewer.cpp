...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case Next_page:
		display_num = (display_num + 1) % file_list.size();
		file_load(g_SPinfo, file_list[display_num]);
		InvalidateRect(hWnd, NULL, true);
		break;
	case Before_page:
		display_num = (file_list.size() + display_num - 1) % file_list.size();
		file_load(g_SPinfo, file_list[display_num]);
		InvalidateRect(hWnd, NULL, true);
		break;
	case WM_CREATE:
	{
		...
		buttons.push_back(new GB_BUTTON(LEFT_text, LEFT, LEFT_x, LEFT_y, LEFT_width, LEFT_height));
		buttons.push_back(new GB_BUTTON(RIGHT_text, RIGHT, RIGHT_x, RIGHT_y, RIGHT_width, RIGHT_height));
		...
		// 디렉터리 확인 및 생성
		if(!SetCurrentDirectory(FILE_PATH))
		{
		...
		hFind = FindFirstFileW(L"*", &data);	// 경로 ./ 와 ../ 제거
		...
		FindClose(hFind);
		display_num = rand() % file_list.size();
		file_load(g_SPinfo, file_list[display_num]);
	...
	case WM_COMMAND:
	{
		...
		case IDM_LOAD:
		{
			OPENFILENAME OFN;
			wchar_t str[256] = { 0, };
			wchar_t file_name[256] = FILE_PATH;
	...
	case WM_LBUTTONDOWN:
		...
		case RIGHT:
			PostMessage(g_hWnd, Next_page, 0, 0);
			Critical_flag(true);
			break;
		case LEFT:
			PostMessage(g_hWnd, Before_page, 0, 0);
			Critical_flag(true);
			break;
		case REPLAY:
...
DWORD WINAPI drawing(LPVOID points)
{
	...
	while (true)
	{
		...
		for (size_t i = 0; i < (int)(g_SPinfo.pinfo.size() - 1); i++)
		{
			...
			//x = BOUNDARY_LEFT + (int)((LOWORD(g_SPinfo.pinfo[i].lparm) - g_SPinfo.x) * ((float)GetSystemMetrics(SM_CXSCREEN)-BOUNDARY_LEFT)  / (float)g_SPinfo.width / 2L);
			//y = BOUNDARY_TOP + (int)((HIWORD(g_SPinfo.pinfo[i].lparm) - g_SPinfo.y) * ((float)GetSystemMetrics(SM_CYSCREEN)-BOUNDARY_TOP) / (float)g_SPinfo.height * 1.7);
			...
			if (g_SPinfo.pinfo[i + 1].state == WM_LBUTTONDOWN)  // 다음벡터도 WM_MOUSEMOVE일 경우에만 sleep 
			{
				Sleep(5);
			}
			else
